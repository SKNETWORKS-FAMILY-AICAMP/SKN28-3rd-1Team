...
from ingest.schemas import RagDocument, SearchResult

SUPPORTED_INPUT_SUFFIXES = {".csv", ".json", ".py", ".txt", ".md"}


def load_documents(input_dir: Path) -> list[RagDocument]:
...
def search_documents(input_dir: Path, query: str, top_k: int) -> list[SearchResult]:
...
def _score(query: str, text: str) -> float:
    ...
    if len(primary_terms) >= 3 and primary_matched < 2:
        return 0.0
