...
- `.toon` 원본 문서를 보존한 상태에서 사람이 검토 가능한 chunk `.toon` 파일을 만든다.
...
`chunks/` 안의 파일은 모두 쪼개진 `.toon` 파일이다. chunk별 metadata만 따로 저장하는 것이 아니라, 원본 내용을 의미 단위로 나눈 `.toon` 조각을 저장한다고 보면 된다.
...
6. `chunks/` 디렉터리를 만든다.
7. 500 토큰 이하를 목표로 의미 단위 chunk `.toon` 파일을 만든다.
...
- 기존 chunk `.toon` 파일을 사용자 확인 없이 덮어쓰지 않는다.
...
- `chunks/` 아래에 사용자 확인을 거친 chunk `.toon` 파일들이 있다.
