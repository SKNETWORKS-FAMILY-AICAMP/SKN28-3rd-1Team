...
| 컬럼 | 설명 | 컬럼 | 설명 |
|---|---|---|---|
| `timestamp` | 실행 시각 | `testcase_id` | testcase 식별자 |
| `query_reference` | 질문, 기대 키워드, 참고 답변, 판정 기준 묶음 | `answer` | 모델 응답 |
| `input_price_per_1m` | input token 100만 개당 가격 | `output_price_per_1m` | output token 100만 개당 가격 |
| `input_tokens` | 입력 token 수 | `output_tokens` | 출력 token 수 |
| `used_tokens` | input과 output을 합친 token 수 | `input_cost_usd` | input token 비용 |
| `output_cost_usd` | output token 비용 | `total_cost_usd` | input과 output을 합친 총 비용 |
| `batch` | testcase 배치 | `difficulty` | testcase 난이도 |
| `special_case` | 특수 케이스 여부 | `model_id` | 실행 모델 ID |
| `provider_order` | 요청한 provider 순서 | `primary_provider_slug` | 1순위 provider slug |
| `primary_provider_quantization` | 1순위 provider quantization | `actual_provider` | 실제 응답한 provider |
| `allow_fallbacks` | fallback 허용 여부 | `endpoint_tools_supported` | endpoint의 tool 지원 여부 |
| `context_length` | context 길이 제한 | `max_completion_tokens` | 최대 completion token |
| `quantization` | 실제 사용 quantization | `openrouter_generation_id` | OpenRouter generation ID |
| `latency_ms` | 응답시간(ms) | `langsmith_project` | LangSmith project |
| `langsmith_tags` | LangSmith tag | `status` | 실행 상태 |
| `error` | 실패 시 에러 메시지 |  |  |
...
1. 숫자 컬럼은 `pd.to_numeric(..., errors="coerce")`로 변환한다.
2. 평균 token 수, 평균 비용, 평균 latency는 `status == "success"` row 기준으로 계산한다.
3. 실패 row는 평균 계산에서 제외하되, `failed_count`와 `failure_report`에 남긴다.
4. provider별 순수 비교는 routing 검증을 통과한 provider만 기준으로 본다.
5. routing 검증 통과 조건은 `allow_fallbacks=false`이고, 성공 row의 `actual_provider`가 `primary_provider_slug`와 일치하는 것이다.
6. `p95_latency_ms`, `p95_used_tokens`는 `numpy.percentile(..., 95)` 기본값인 linear 보간 방식으로 계산한다.
...
- 표의 `#번호`는 차트 x축과 후보 판단표에서 같은 strict 후보를 가리킨다. 
- 차트 x축은 번호와 provider 이름만 표시하고, 모델 구분은 범례 색상으로 한다.
...
<table>
  <thead>
    ...
    <tr>
      ...
      <th>모델</th>
      <th>제공자</th>
      <th align="right">성공/전체</th>
      <th align="right">실패 수</th>
      <th align="right">평균 input token</th>
      <th align="right">평균 output token</th>
      <th align="right">평균 used token</th>
      <th align="right">평균 비용 USD</th>
      <th align="right">총 비용 USD</th>
      <th align="right">평균 응답시간(ms)</th>
      <th align="right">95% 이내 응답시간(ms)</th>
    </tr>
  </thead>
  <tbody>
    <tr>
    ...
    <tr>
      ...
      <td align="right">360/360</td>
      <td align="right">0</td>
      <td align="right"><mark>941.8</mark></td>
    ...
    <tr>
      ...
      <td align="right">0</td>
      <td align="right">936.4</td>
      <td align="right"><mark>1,542.5</mark></td>
...
차트별 핵심만 보면(차트 라벨과 같은 반올림 표기):

- 평균 비용이 가장 저렴한 후보는 `#2 deepseek-v4-flash / deepinfra`, `$0.000198`이다.
- 평균 latency가 가장 낮은 후보는 `#1 gpt-oss-120b / cerebras`, `1.4s`이다.
- 평균 used token이 가장 낮은 후보는 `#2 deepseek-v4-flash / deepinfra`, `1,462`이다.
- 평균 input token이 가장 높은 후보는 `#8 deepseek-v4-pro / gmicloud`, `942`이다.
- 평균 output token이 가장 높은 후보는 `#9 deepseek-v4-pro / alibaba`, `1,542`이다.

## 1. 평균 비용 비교

![Average cost per question](charts/avg_cost_per_question.png)

- 이 차트는 성공 질문 1개당 평균 비용을 보여준다. 
- 같은 색은 같은 모델 계열이다. 
- 비용 차트는 막대가 낮을수록 평균 비용이 저렴하다.
- 차트 막대 위 라벨은 `$` 기준 소수점 6자리로 반올림한 값이다. 

차트 기준 평균 비용이 저렴한 순서:

- 1위는 `#2 deepseek-v4-flash / deepinfra`: `$0.000198`
- 2위는 `#5 deepseek-v4-flash / gmicloud`: `$0.000306`
- 3위는 `#3 deepseek-v4-flash / deepseek`: `$0.000351`
- 4위는 `#4 deepseek-v4-flash / siliconflow`: `$0.000392`
- 5위는 `#1 gpt-oss-120b / cerebras`: `$0.001123`

평균 비용만 보면 `#2 deepseek-v4-flash / deepinfra`가 가장 저렴하다. (속도나 p95 안정성 판단 미포함)

## 2. 평균 latency 비교

![Average latency](charts/avg_latency_ms.png)

- 이 차트는 성공 질문 1개당 평균 응답 시간을 보여준다. 
- 막대 위 숫자는 `latency_ms`를 초 단위로 바꾼 뒤 소수점 1자리로 반올림한 평균 latency다. 

차트 기준 평균 latency가 낮은 순서:

- 1위는 `#1 gpt-oss-120b / cerebras`: `1.4s`
- 2위는 `#3 deepseek-v4-flash / deepseek`: `12.4s`
- 3위는 `#4 deepseek-v4-flash / siliconflow`: `14.5s`
- 4위는 `#5 deepseek-v4-flash / gmicloud`: `16.6s`
- 5위는 `#6 deepseek-v4-pro / streamlake`: `17.4s`

## 3. 평균 input token 비교

![Average input tokens](charts/avg_input_tokens.png)

- 이 차트는 provider별 평균 input token을 보여준다.
- input token이 크면 같은 답변 길이라도 기본 비용이 올라간다.
-  차트 막대 위 라벨은 평균 token 값을 정수로 반올림한 값이다.

차트 기준 평균 input token이 낮은 순서:

- 1위는 `#1 gpt-oss-120b / cerebras`: `894`
- 2위는 `#2 deepseek-v4-flash / deepinfra`: `936`
- 3위는 `#3 deepseek-v4-flash / deepseek`: `936`
- 4위는 `#4 deepseek-v4-flash / siliconflow`: `936`
- 5위는 `#6 deepseek-v4-pro / streamlake`: `936`

## 4. 평균 output token 비교

![Average output tokens](charts/avg_output_tokens.png)

- 이 차트는 provider별 평균 output token을 보여준다.
- output token은 비용과 답변 길이에 직접 영향을 준다. 
- 차트 막대 위 라벨은 평균 token 값을 정수로 반올림한 값이다.

차트 기준 평균 output token이 낮은 순서:

- 1위는 `#2 deepseek-v4-flash / deepinfra`: `526`
- 2위는 `#1 gpt-oss-120b / cerebras`: `1,081`
- 3위는 `#5 deepseek-v4-flash / gmicloud`: `1,160`
- 4위는 `#4 deepseek-v4-flash / siliconflow`: `1,187`
- 5위는 `#3 deepseek-v4-flash / deepseek`: `1,223`

## 5. 평균 used token 비교

![Average used tokens](charts/avg_used_tokens.png)

- 이 차트는 input과 output을 합친 평균 used token 수를 보여준다. 
- 미세한 차이를 보기 위해 y축을 확대했다. 
- 차트 막대 위 라벨은 평균 token 값을 정수로 반올림한 값이다.

차트 기준 평균 used token이 낮은 순서:

- 1위는 `#2 deepseek-v4-flash / deepinfra`: `1,462`
- 2위는 `#1 gpt-oss-120b / cerebras`: `1,975`
- 3위는 `#5 deepseek-v4-flash / gmicloud`: `2,098`
- 4위는 `#4 deepseek-v4-flash / siliconflow`: `2,124`
- 5위는 `#3 deepseek-v4-flash / deepseek`: `2,159`
...
- 이 차트는 provider별 평균 비용과 평균 latency를 함께 보여준다.
- 왼쪽 아래에 가까울수록 비용과 속도 측면에서 유리하다. 
- 이 산점도는 점 옆에 숫자 값을 직접 쓰지 않고 `#번호 + provider`만 표시하므로, 정확한 비용과 latency 값은 위 표나 각 차트 아래 설명을 기준으로 확인한다.

## 7. latency 분포 비교

![Latency distribution](charts/latency_distribution_boxplot.png)

- 이 차트는 provider별 latency 분포를 보여준다. 
- 박스 위 숫자는 `latency_ms`의 median을 초 단위로 바꾼 뒤 소수점 1자리로 반올림한 값이다. 
- 박스 자체는 CSV의 원본 `latency_ms` 분포를 기준으로 그렸다.
