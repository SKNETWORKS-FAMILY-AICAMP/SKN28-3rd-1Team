...
def _openrouter_headers() -> dict[str, str]:
    headers = {
        "X-Title": settings.llm.openrouter_app_title,
    }

    if settings.llm.openrouter_app_url:
        headers["HTTP-Referer"] = settings.llm.openrouter_app_url
...
def _openrouter_extra_body() -> dict[str, Any]:
    provider: dict[str, Any] = {
        "allow_fallbacks": settings.llm.openrouter_allow_fallbacks,
    }

    if settings.llm.openrouter_provider_order:
        provider["order"] = settings.llm.openrouter_provider_order

    if settings.llm.openrouter_require_parameters:
...
def _reasoning_effort() -> str | None:
    if settings.llm.reasoning_effort is None:
        return None

    value = settings.llm.reasoning_effort.strip()
...
def get_chat_llm() -> ChatOpenAI:
    ...
    logger.info(
        ...
        settings.llm.openrouter_provider_order,
        settings.llm.openrouter_allow_fallbacks,
    )
    ...
    if settings.llm.max_tokens:
        llm_kwargs["max_completion_tokens"] = settings.llm.max_tokens
...
def get_speech_text_llm() -> ChatOpenAI:
    ...
    logger.info(
        ...
        settings.llm.speech_text_model,
    )

    llm_kwargs: dict[str, Any] = {
        "model": settings.llm.speech_text_model,
        "api_key": settings.llm.openrouter_api_key.get_secret_value(),
        "base_url": settings.llm.openrouter_base_url,
        "temperature": settings.llm.speech_text_temperature,
        "timeout": settings.llm.timeout_ms / 1000,
        "max_retries": settings.llm.max_retries,
        "default_headers": _openrouter_headers(),
        "extra_body": _openrouter_extra_body(),
    }
    if settings.llm.speech_text_max_tokens:
        llm_kwargs["max_completion_tokens"] = settings.llm.speech_text_max_tokens
