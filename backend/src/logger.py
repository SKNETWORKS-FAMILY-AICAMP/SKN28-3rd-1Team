...
class StructuredLogFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if record.stack_info:
            masked_fields["stack_info"] = self.formatStack(record.stack_info)
...
def configure_logging() -> None:
    ...
    logging.basicConfig(
        level=_log_level(settings.runtime.log_level),
        handlers=[handler],
        force=True,
    )
...
def _configure_third_party_loggers() -> None:
    for logger_name in ("daphne.cli", "daphne.server", "httpx", "httpcore"):
