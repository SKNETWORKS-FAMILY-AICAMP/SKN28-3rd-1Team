...
import { Briefcase, Clock, Coins, HeartHandshake, Scale, ShieldAlert } from "lucide-react";
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {categories.map((category, index) => (
          <FadeIn key={category.title} delay={index * 80}>
            <Link
              href={`/chat?q=${encodeURIComponent(category.q)}`}
            ...
            >
              ...
              <div>
                <h3 className="font-heading text-lg text-foreground">{category.title}</h3>
                <p className="line-clamp-1 text-sm text-muted-foreground">{category.q}</p>
              </div>
            </Link>
          </FadeIn>
        ))}
