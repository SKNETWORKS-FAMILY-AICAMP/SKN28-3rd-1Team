import { MessageSquareText, PencilLine, Search } from "lucide-react";
...
export function HowItWorks() {
  return (
    <section id="how" className="border-y border-border/5 bg-gradient-to-b from-background via-secondary/20 to-background">
          ...
          {steps.map((step, index) => (
            <FadeIn key={step.step} delay={index * 150}>
              <div className="relative rounded-3xl border border-border bg-card p-8 shadow-sm">
                <span className="font-heading text-5xl text-primary/25">{step.step}</span>
                <span className="mt-2 flex size-12 items-center justify-center rounded-2xl bg-primary text-primary-foreground">
                  <step.icon className="size-6" />
                </span>
                <h3 className="mt-5 font-heading text-xl text-foreground">{step.title}</h3>
