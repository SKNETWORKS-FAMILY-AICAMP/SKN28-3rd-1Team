...
export function useBrowserSpeechDictation({
  onTranscriptChange,
}: UseBrowserSpeechDictationOptions = {}) {
...
function getSpeechErrorMessage(error?: string) {
  ...
  if (error === "no-speech") return "음성이 들리지 않았어요.";
  if (error === "audio-capture") return "마이크를 찾지 못했어요.";
  if (error === "network") return "음성 인식 네트워크 오류가 발생했어요.";
